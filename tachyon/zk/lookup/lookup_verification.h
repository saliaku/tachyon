...
std::vector<crypto::PolynomialOpening<Poly, C>> CreateLookupQueries(
  ...
  queries.emplace_back(base::ShallowRef<const C>(data.product_commitment),
                       base::DeepRef<const F>(data.x), *data.product_eval);
  // Open lookup input commitments at x.
  queries.emplace_back(
      base::ShallowRef<const C>(&data.permuted_commitment->input()),
      base::DeepRef<const F>(data.x), *data.permuted_input_eval);
  // Open lookup table commitments at x.
  queries.emplace_back(
      base::ShallowRef<const C>(&data.permuted_commitment->table()),
