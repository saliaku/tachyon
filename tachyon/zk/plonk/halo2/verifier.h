...
class Verifier : public VerifierBase<PCS> {
 ...
 private:
  ...
  FRIEND_TEST(SimpleLookupCircuitTest, VerifyMultiProof);
  ...
  void CreateColumnQueries(const std::vector<QueryData<C>>& queries,
    ...
    for (size_t i = 0; i < queries.size(); ++i) {
      ...
      openings.emplace_back(
          base::ShallowRef<const Commitment>(&commitments[column.index()]),
  ...
  bool DoVerify(
    ...
    for (size_t i = 0; i < proof.common_permutation_evals.size(); ++i) {
      queries.emplace_back(base::ShallowRef<const Commitment>(
                               &common_permutation_commitments[i]),
                           base::DeepRef<const F>(&proof.x),
                           proof.common_permutation_evals[i]);
    }
    ...
    queries.emplace_back(base::ShallowRef<const Commitment>(&h_commitment),
