#ifndef TACHYON_ZK_PLONK_CIRCUIT_EXAMPLES_FIBONACCI2_CIRCUIT_H_
#define TACHYON_ZK_PLONK_CIRCUIT_EXAMPLES_FIBONACCI2_CIRCUIT_H_
...
struct Fibonacci2Config {
  ...
  Fibonacci2Config(const AdviceColumnKey& advice, const Selector& selector,
                   const InstanceColumnKey& instance)
      : advice(advice), selector(selector), instance(instance) {}

  Fibonacci2Config Clone() const {
    return Fibonacci2Config(advice, selector, instance);
  }
...
class Fibonacci2Chip {
 public:
  explicit Fibonacci2Chip(Fibonacci2Config<F>&& config)
...
#endif  // TACHYON_ZK_PLONK_CIRCUIT_EXAMPLES_FIBONACCI2_CIRCUIT_H_
