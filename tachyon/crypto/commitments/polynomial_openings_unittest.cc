...
using PolyShallowRef = base::ShallowRef<const Poly>;
...
TEST_F(PolynomialOpeningsTest, GroupByPolyOracleAndPoints) {
  ...
  poly_openings.emplace_back(PolyShallowRef(&polys_[0]),
                             PointDeepRef(&points_[0]),
                             polys_[0].Evaluate(points_[0]));
  poly_openings.emplace_back(PolyShallowRef(&polys_[0]),
                             PointDeepRef(&points_[1]),
                             polys_[0].Evaluate(points_[1]));
  poly_openings.emplace_back(PolyShallowRef(&polys_[1]),
                             PointDeepRef(&points_[0]),
                             polys_[0].Evaluate(points_[0]));
  poly_openings.emplace_back(PolyShallowRef(&polys_[1]),
                             PointDeepRef(&points_[1]),
                             polys_[0].Evaluate(points_[1]));
  poly_openings.emplace_back(PolyShallowRef(&polys_[2]),
  ...
  for (const auto& poly_oracle_grouped_pair : poly_openings_grouped_by_poly) {
    ...
    if (poly_oracle_grouped_pair.poly_oracle == PolyShallowRef(&polys_[0]) ||
        poly_oracle_grouped_pair.poly_oracle == PolyShallowRef(&polys_[1])) {
      expected_points = {
          PointDeepRef(&points_[0]),
          PointDeepRef(&points_[1]),
      };
  ...
  for (const auto& point_grouped_pair :
    ...
    std::vector<PolyShallowRef> expected_polys;
